...
\subsubsection{Ionotropic and Metabotropic Receptors}

The important difference lies in what happens after dopamine binds. Every synapse in the direct and indirect pathways used an \textbf{ionotropic} receptor: the receptor \textit{is} an ion channel, the transmitter binding opens it, ions flow, and the membrane potential changes within a millisecond or so. Glutamate opens cation channels and depolarises; GABA opens chloride channels and hyperpolarises. Fast, local, unambiguous.

Dopamine receptors are not ion channels at all. They are \textbf{metabotropic} receptors, and specifically they belong to the enormous family of \textbf{G protein-coupled receptors (GPCRs)}. A dopamine receptor cannot pass a current. What it can do is start a chain of chemistry inside the cell that changes how that cell responds to everything else arriving at it.

\subsubsection{How a GPCR Works}

A GPCR is a single protein that threads back and forth through the membrane seven times, which is why the family is sometimes called the seven-transmembrane receptors. Its binding site faces outward; its business end faces the cytoplasm. The sequence of events runs as follows.
...
\begin{enumerate}
    ...
    \item The GTP-bound $\alpha$ subunit separates from the $\beta\gamma$ pair and goes off to act on an \textbf{effector enzyme}. For dopamine receptors that enzyme is \textbf{adenylyl cyclase}.
    \item Adenylyl cyclase manufactures a \textbf{second messenger}, \textbf{cyclic AMP (cAMP)}, which diffuses through the cytoplasm.
    \item cAMP activates \textbf{protein kinase A (PKA)}, an enzyme that phosphorylates other proteins, including the ion channels already sitting in the membrane. Phosphorylation changes how readily those channels open.
    \item The $\alpha$ subunit eventually hydrolyses its GTP back to GDP, reassembles with $\beta\gamma$, and the system resets.
\end{enumerate}

Three consequences of this arrangement matter for what follows. It is \textit{slow}: a cascade of several enzymatic steps takes hundreds of milliseconds to seconds, against the millisecond of an ionotropic synapse. It is \textit{amplifying}: one bound receptor activates many G proteins, each activated enzyme makes many molecules of cAMP, and so a small amount of transmitter produces a large intracellular effect. And it is \textit{modulatory} rather than commanding: the end result is not that the cell fires, but that the channels it already has are tuned to open more or less readily. A GPCR does not send a message; it changes how the cell reads its other messages.
