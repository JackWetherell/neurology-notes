...
The basal ganglia are a set of subcortical nuclei buried deep within the cerebral hemispheres, sitting at the crossroads between cortex and brainstem. They are not part of the cortex, not part of the thalamus, and not part of the cerebellum, but they communicate intimately with all three. Their fundamental job is \textbf{action selection}: out of the many possible motor programmes the cortex is simultaneously preparing, the basal ganglia decide which one gets released into execution and which ones get suppressed. They are, in essence, a gate, and their dysfunction produces either too little movement (as in Parkinson's disease) or too much of the wrong kind (as in Huntington's disease and other hyperkinetic disorders).

This gating function extends beyond skeletal movement. The same circuitry that selects which limb movement to release also selects which thought to pursue, which eye movement to make, and which emotional impulse to act on. The basal ganglia are therefore implicated not only in movement disorders but also in obsessive-compulsive disorder, addiction, Tourette syndrome, and aspects of schizophrenia. Understanding their wiring is one of the clearest examples in neuroscience of how a single circuit motif, repeated across parallel loops, can explain an extraordinarily wide range of clinical phenomena.

\subsection{Components and Structure}

The basal ganglia comprise several distinct nuclei, each with a specific role in the circuit. The terminology can be confusing because the same structures have accumulated multiple names over the decades, but the core components are these:

\begin{itemize}
    \item The \textbf{striatum} is the main input nucleus of the basal ganglia, the structure that receives the bulk of cortical and thalamic afferents. It is divided by the \textbf{internal capsule} (a massive white-matter tract carrying cortical output) into two parts: the \textbf{caudate nucleus}, which arches along the lateral ventricle and is more associated with cognitive and oculomotor loops, and the \textbf{putamen}, which sits laterally and is more associated with motor loops. Despite the physical separation, caudate and putamen are histologically identical and functionally continuous; where they remain connected ventrally the tissue is called the \textbf{nucleus accumbens}, the ventral striatum's key node for reward and motivation.
    \item The \textbf{globus pallidus} (GP) sits medial to the putamen and is divided into an \textbf{external segment (GPe)} and an \textbf{internal segment (GPi)}. The GPi is one of the two main output nuclei of the basal ganglia; its neurons fire tonically and exert a constant inhibitory brake on the thalamus.
    \item The \textbf{subthalamic nucleus} (STN) is a small, lens-shaped nucleus located just below the thalamus. Unlike every other basal ganglia nucleus, it is \textit{excitatory} (glutamatergic), and it plays a crucial role in strengthening the suppression of unwanted movements. It is also the most common target for deep brain stimulation in Parkinson's disease.
    \item The \textbf{substantia nigra} lies in the midbrain and has two parts. The \textbf{pars compacta (SNc)} contains dopaminergic neurons that modulate striatal activity; their degeneration is the defining pathology of Parkinson's disease. The \textbf{pars reticulata (SNr)} is functionally and histologically similar to the GPi and serves as the second main output nucleus of the basal ganglia, particularly for oculomotor and limbic circuits.
\end{itemize}

The overall flow of information through the basal ganglia forms a loop: \textbf{cortex $\rightarrow$ striatum $\rightarrow$ GP/SNr $\rightarrow$ thalamus $\rightarrow$ cortex}. The cortex excites the striatum; the striatum inhibits the output nuclei (GPi/SNr); the output nuclei tonically inhibit the thalamus; and the thalamus excites the cortex. The key insight is that the output nuclei are \textit{always on}, always suppressing the thalamus, and the striatum's job is to \textit{release} that brake for the currently selected action. This double-inhibitory arrangement (striatum inhibits GPi, which inhibits thalamus) means that striatal firing results in \textit{disinhibition} of the thalamus, effectively opening the gate.

\subsection{Direct Pathway}

The \textbf{direct pathway} is the circuit through which the basal ganglia \textit{facilitate} a selected movement. Its logic is elegant in its simplicity: two inhibitory synapses in series produce net excitation.

The pathway runs as follows:

\begin{enumerate}
    \item The \textbf{cortex} sends excitatory (glutamatergic) projections to the \textbf{striatum}.
    \item A population of striatal medium spiny neurons (MSNs) expressing \textbf{D1 dopamine receptors} is activated. These neurons project directly to the \textbf{GPi/SNr}.
    \item Because MSNs are GABAergic (inhibitory), their firing \textit{inhibits} the tonically active GPi/SNr neurons.
    \item With the GPi/SNr silenced, their tonic inhibition of the \textbf{ventral anterior (VA)} and \textbf{ventral lateral (VL)} nuclei of the thalamus is lifted.
    \item The thalamus, now disinhibited, sends excitatory projections back up to the cortex, facilitating the motor programme that the cortex was preparing.
\end{enumerate}

The net effect is that cortical activation of the direct pathway \textit{releases} a movement. When you decide to reach for a glass, it is the direct pathway that lifts the thalamic brake and allows the motor cortex to drive the movement through to execution.

The direct pathway MSNs also co-express the neuropeptides \textbf{substance P} and \textbf{dynorphin}, which serve as additional markers distinguishing them from indirect pathway neurons. Importantly, dopamine acting on D1 receptors \textit{excites} these neurons, meaning that dopamine biases the system toward movement facilitation, a fact that becomes clinically devastating when dopamine is lost.

\subsection{Indirect Pathway}

The \textbf{indirect pathway} is the circuit through which the basal ganglia \textit{suppress} competing movements, those motor programmes that are not currently selected and must be held in check so that the chosen action can proceed cleanly. Where the direct pathway opens the gate, the indirect pathway reinforces its closure.

The pathway runs as follows:

\begin{enumerate}
    \item The \textbf{cortex} sends excitatory projections to the \textbf{striatum}.
    \item A different population of striatal MSNs, expressing \textbf{D2 dopamine receptors}, is activated. These neurons project to the \textbf{GPe} (not directly to GPi).
    \item The D2 MSNs inhibit the GPe.
    \item With the GPe silenced, its tonic inhibition of the \textbf{subthalamic nucleus} (STN) is lifted.
    \item The STN, now disinhibited, fires excitatory (glutamatergic) projections to the \textbf{GPi/SNr}.
    \item The GPi/SNr, now driven more strongly, \textit{increases} its inhibition of the thalamus.
    \item The thalamus is more powerfully suppressed, and the corresponding motor programme is held back.
\end{enumerate}

The net effect is the opposite of the direct pathway: cortical activation of the indirect pathway \textit{strengthens the thalamic brake} and suppresses movement. The indirect pathway MSNs co-express the neuropeptide \textbf{enkephalin} and are \textit{inhibited} by dopamine acting at D2 receptors. This means dopamine simultaneously facilitates the direct pathway (via D1) and suppresses the indirect pathway (via D2), both actions converging to promote movement.

This dual action of dopamine explains the cardinal features of Parkinson's disease. When SNc dopamine neurons degenerate, the direct pathway becomes underactive (less facilitation) and the indirect pathway becomes overactive (more suppression). The result is the classic triad: \textbf{bradykinesia} (slowness), \textbf{rigidity} (increased tone from tonic suppression), and \textbf{resting tremor} (from oscillatory instability in the under-damped circuit). Conversely, excess dopaminergic stimulation, whether from medication or from pathology, can tip the balance toward the direct pathway and produce \textbf{dyskinesias}: involuntary, excessive movements.

\subsection{Modulatory Pathway}

Beyond the classical direct and indirect pathways, the basal ganglia are profoundly shaped by \textbf{modulatory inputs}, chief among them the dopaminergic projection from the substantia nigra pars compacta. This nigrostriatal pathway is not a third ``pathway'' in the same architectural sense as the direct and indirect routes; rather, it is a gain-control system that biases the balance between them. Nevertheless, it is so central to basal ganglia function, and so clinically important, that it warrants treatment as a pathway in its own right.

\subsubsection{The Nigrostriatal Dopamine System}

The \textbf{substantia nigra pars compacta (SNc)} contains a dense population of dopaminergic neurons whose axons ascend to innervate the entire dorsal striatum. These neurons do not carry specific motor commands; instead, they broadcast a modulatory signal that adjusts the excitability of striatal MSNs according to whether an action's outcome was better or worse than expected. This is the famous \textbf{reward prediction error} signal identified in the work of Wolfram Schultz and others: dopamine neurons fire in bursts when an outcome is unexpectedly good, pause when an outcome is unexpectedly bad, and maintain a tonic baseline when the world unfolds as predicted.

The effect of this dopamine signal on the two MSN populations is opposite and complementary:

\begin{itemize}
    \item \textbf{D1 receptors} (on direct pathway MSNs) are coupled to G\textsubscript{s} proteins that increase neuronal excitability. When dopamine rises, direct pathway neurons become more responsive to cortical input, making it easier to release the selected action.
    \item \textbf{D2 receptors} (on indirect pathway MSNs) are coupled to G\textsubscript{i} proteins that decrease neuronal excitability. When dopamine rises, indirect pathway neurons become less responsive, weakening the suppression of competing actions.
\end{itemize}

The net result is that dopamine tilts the entire system toward action and away from inhibition. It does not specify \textit{which} action to perform, that is determined by which cortical inputs are active, but it adjusts the \textit{threshold} for action release. High dopamine lowers the gate; low dopamine raises it.

\subsubsection{Plasticity and Learning}

Dopamine's modulatory role extends beyond moment-to-moment gain control into the domain of \textbf{synaptic plasticity} and motor learning. The corticostriatal synapses onto MSNs undergo long-term potentiation (LTP) and long-term depression (LTD) in a dopamine-dependent manner:

\begin{itemize}
    \item At D1 MSN synapses, coincident cortical activity and a dopamine burst (signalling a positive prediction error) strengthen the synapse, making it more likely that the same cortical pattern will activate the direct pathway in future, reinforcing the successful action.
    \item At D2 MSN synapses, dopamine dips (signalling a negative prediction error) strengthen the synapse, making it more likely that the indirect pathway will suppress the action that led to a bad outcome.
\end{itemize}

This is the cellular basis of \textbf{reinforcement learning} in the basal ganglia. Over many trials, the striatum learns which cortical patterns should be facilitated and which should be suppressed, gradually shaping the repertoire of habitual actions. It is why the basal ganglia are essential not only for executing movements but for \textit{learning} which movements to make, and why their dysfunction produces not only motor symptoms but also deficits in habit formation, procedural learning, and decision-making under uncertainty.

\subsubsection{Other Modulatory Inputs}

Dopamine is the dominant modulator, but the striatum also receives important modulatory afferents from other systems:

\begin{itemize}
    \item \textbf{Cholinergic interneurons} within the striatum (the tonically active neurons, or TANs) provide local acetylcholine that modulates MSN excitability and synaptic plasticity. They pause their firing in response to salient stimuli, complementing the dopamine burst and helping to gate learning.
    \item \textbf{Serotonergic projections} from the dorsal raph\'{e} nuclei modulate impulsivity and the balance between waiting and acting, relevant to the pathophysiology of both depression and impulse-control disorders.
    \item \textbf{Glutamatergic projections} from the \textbf{centromedian and parafascicular nuclei} of the thalamus provide a second major excitatory input to the striatum, one that is thought to signal attentional shifts and arousal rather than specific motor plans.
\end{itemize}

Together, these modulatory systems ensure that basal ganglia output is not a fixed reflex arc but a \textit{context-sensitive, learnable} selection process, one that integrates reward history, arousal, and motivational state into every decision about whether to act or to wait.
